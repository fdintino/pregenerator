      ...
      validate: chain(
        ...
        Object.assign(
          function (node, key, val) {
            ...
            if (val && has(node, "computed") && node.computed) {
              throw new TypeError(
        ...
        function (node, key, val) {
          ...
          if (val && !(has(node, "key") && is("Identifier", node.key))) {
            throw new TypeError(
    ...
    declaration: {
      ...
      validate: chain(
        ...
        function (node, key, val) {
          ...
          if (val && has(node, "source") && node.source) {
  ...
  key: {
    validate: chain(
      (function () {
        ...
        return function (node: Node, key: string, val: any) {
          const validator = (has(node, "computed") && node.computed) ? computed : normal;
          validator(node, key, val);
        };
      })(),
      assertNodeType(
